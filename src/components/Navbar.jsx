...
const Navbar = () => {
    ...
    const { user, setUser, setShowUserLogin, nagigate } = useAppContext();
        ...
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative z-50 transition-all">
                ...
                {!user ? (
                    ...
                    <div className='relative group'>
                        ...
                        <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40'>
                            <Link to="/my-orders"><li className='p-1 pl-3 hover:bg-primary/10 cursor-pointer'>My Orders</li></Link>
            ...
            {open && (
                <div className="absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex flex-col items-start gap-2 px-5 text-sm md:hidden z-50">
                    ...
                    <Link to="/all-product" onClick={() => setOpen(false)}>All Product</Link>
                    {user && (
